  ...
  return {
    ...
    execute: function () {
      ...
      var rabbitSlotCtrl = exports('rabbitSlotCtrl', (_dec = ccclass('rabbitSlotCtrl'), _dec2 = property({
        ...
        // private isClickBigWin: boolean = false;
        // onBigWinClick() {
        //     if (!(this.watchLabel && this.watchLabel.node)) return;
        //     Tween.stopAllByTarget(this.watchLabel.node);
        //     if (this.isClickBigWin) {
        //         return;
        //     }
        //     this.isClickBigWin = true;
        //     this.watchLabel.scheduleOnce(() => {
        //         this.isClickBigWin = false;
        //     }, 1);

        //     if (this.inumObj.inum >= this.score_bigwin) {
        //         this.closeBigWin()
        //     } else {
        //         this.inumObj.inum = this.score_bigwin//this.getNextTargetNum_bigWin()
        //         this.showBigWinByRate();
        //     }
        ...
        // //不更换背景只换文字的大奖动画
        // playBigWin_special(coin: number = 0) {
        //     this.bigWinNode.active = true;
        //     (this.bigWinNode.children[0].getChildByName("bw") as Node).active = true;
        //     (this.bigWinNode.children[1].getChildByName("bw") as Node).active = false;
        //     (this.bigWinNode.children[2].getChildByName("bw") as Node).active = false;
        //     (this.bigWinNode.children[0].getComponent(Animation) as Animation).play();
        //     this.playBigWinCoin(coin);
        //     let addcoin = 0;
        //     this.bigWinCoin_lab.string = Num.exchangeToThousands(rabbit.exchangeRate, addcoin);
        //     this.schedule(() => {
        //         addcoin += coin / 100;
        //         if (addcoin > coin) {
        //             addcoin = coin;
        //         }
        //         if (addcoin >= rabbit.betSum * 50 && !this.bigWinNode.children[2].active) {
        //             (this.bigWinNode.children[0].getChildByName("bw") as Node).active = false;
        //             (this.bigWinNode.children[1].getChildByName("bw") as Node).active = false;
        //             (this.bigWinNode.children[2].getChildByName("bw") as Node).active = true;
        //             (this.bigWinNode.children[2].getComponent(Animation) as Animation).play();
        //         } else if (addcoin >= rabbit.betSum * 35 && addcoin < rabbit.betSum * 50 && !this.bigWinNode.children[1].active) {
        //             (this.bigWinNode.children[0].getChildByName("bw") as Node).active = false;
        //             (this.bigWinNode.children[1].getChildByName("bw") as Node).active = true;
        //             (this.bigWinNode.children[2].getChildByName("bw") as Node).active = false;
        //             (this.bigWinNode.children[1].getComponent(Animation) as Animation).play();
        //         }
